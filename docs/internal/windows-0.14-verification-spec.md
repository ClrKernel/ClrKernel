...
Commit the results file on a branch `windows/0.14-results` and DO NOT push it; do not
open a PR — the person reading it will.

---

## Results — Windows run, 2026-09-16 (written back by the verification session)

Full record, in the §R format: **`docs/internal/windows-0.14-results.md`**, committed on local branch `windows/0.14-results` (not pushed, no PR). Unattended session, not elevated. VS Code UI, Windows Hello and the Credential Manager UI were not operated. Where possible the same behaviour was checked at protocol level (`clrkernel lsp` / `serve`) or through Studio's API with Playwright, and labelled so.

**Suite:** format clean; Release build 0 warnings; Database 182 / Language 243 / Core 226 / Studio 482 passed, 0 failed (net9.0 and net10.0 Language also green); extension 88/88; web app 541/542 (Windows-only test bug, fixed); live SQL Server tiers green; `WindowsCredentialTest` cmdkey = found-and-equal; `server_harness` 10/10, `lsp_harness` 17/17; Jupyter smoke/completion green; Studio browser suite 18/18 once the harness ran on Windows.

**Findings (most severe first; branch = fix committed on it, off `v0.14.0`):**
1. Every Analysis Services processing call (TOM) throws `TypeLoadException … WindowsDesktopAssemblyResolver` on Windows. Branch `windows/ssas-tom-package`.
2. A secret a cell throws with lands unredacted in logs: Studio `run.log` on disk, `clrkernel run` stderr, the `lsp` output channel. Branch `windows/redact-logged-exceptions`.
3. The ODBC opt-in provider cannot open anything via `#r "nuget:"` (System.Data.Odbc 9.0 against the kernel's 8.0.1). Branch `windows/odbc-provider-version`.
4. Saving a `.dib` in VS Code corrupts `#!sql-connect …` / `#!value --name …` lines. Branch `windows/dib-directive-prefix`.
5. Python is never provisioned on a stock Windows box: the Store `python.exe` alias is taken as the interpreter. Reported.
6. A Git Bash `cd` does not persist to the next cell. Branch `windows/git-bash-cwd`.
7. Studio Connections: a non-SqlServer connection with no password (ODBC `Trusted_Connection`) fails with "No secret found". Reported.
8. `lsp`: a second notebook in another folder cannot see its own non-SqlServer `connections.json` entries (process-cwd lookup). Reported.
9. The Studio browser suite could not run on Windows (`build.sh`, `killpg`, `Meta+A`, a 4 s wait). Branch `windows/studio-harness`.
10. `#!sql-connect --provider Odbc` is accepted and silently ignored. Reported.
11. `PRINT`/info messages never reach notebook SQL cells. Reported.
12. `#!sql-deploy --dry-run` says "0 planned". Branch `windows/sql-deploy-dryrun-count`.
13. Web app `noLiterals.test.ts` fails on Windows paths. Branch `windows/webapp-nolit-paths`.
