...
export function ConnectionForm({
  ...
  async function test() {
    if (connection == null) {
      setTesting('Save it first — testing opens the connection the server has stored.');
      return;
    }
    setTesting('Connecting…');
    try {
      const reply = await api.testConnection(connection.id, password || undefined);
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between gap-4">
          <h2 style={{ margin: 0 }}>{connection == null ? 'New connection' : connection.name}</h2>
          ...
          {!prompt && canPersist && (
            <label className="form-field">
              ...
              <Input type="password" value={password} onChange={(e) => setPassword(e.target.value)}
                placeholder={connection?.secretConfigured ? 'leave blank to keep the stored one' : ''} />
              <em className="text-base text-muted-foreground">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" onClick={test} disabled={connection == null}>
            Test
          </Button>
