...
describe('compareKernelVersion', () => {
    it('accepts the supported line, however many parts the version has', () => {
        ...
        expect(compareKernelVersion('0.12.0.0')).toBe('ok');
        expect(compareKernelVersion('0.12.0')).toBe('ok');
    });

    it('treats a patch release of the same line as compatible', () => {
        expect(compareKernelVersion('0.12.7.0')).toBe('ok');
    });

    it('flags a newer kernel — the case this guard exists for', () => {
        expect(compareKernelVersion('0.13.0.0')).toBe('newer');
    ...
    it('flags the previous line as older — 0.11 has no Python cell language', () => {
        expect(compareKernelVersion('0.11.0.0')).toBe('older');
        expect(compareKernelVersion('0.11.9')).toBe('older');
    });
...
describe('kernelVersionWarning', () => {
    it('tells the user what is installed and how to get back', () => {
        const warning = kernelVersionWarning('newer', '0.13.0.0');
        expect(warning).toContain('0.13.0.0');
    ...
    it('says cells still run, because they do — only connection management breaks', () => {
        expect(kernelVersionWarning('newer', '0.13.0.0')).toContain('still run');
    });

    it('asks for an update when the kernel is behind', () => {
        expect(kernelVersionWarning('older', '0.11.0.0')).toContain('dotnet tool update');
    });

    it('stays quiet when the pair is fine or unknown', () => {
        expect(kernelVersionWarning('ok', '0.12.1.0')).toBeUndefined();
