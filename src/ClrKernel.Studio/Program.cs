...
/// <summary>
/// The <c>clrkernel-studio</c> command-line tool. Phase 1 verbs: <c>run</c> (one job,
...
public static class Program {
    private const string _usage =
        """
        clrkernel-studio — notebook job runner for ClrKernel (preview).

        Usage: clrkernel-studio <command> [options]
    ...
    public static async Task<int> Main(string[] args) {
        ...
        switch (command) {
            ...
            case "run":
                if (jobName == null) {
                    Console.Error.WriteLine("run needs a job name. See `clrkernel-studio --help`.");
                    return 2;
                }
                return await RunAsync(projects, options, jobName);
            default:
                Console.Error.WriteLine($"Unknown command: {command}. See `clrkernel-studio --help`.");
                return 2;
        }
    }
    ...
    private static int GitCommand(JobsOptions options, string subVerb) {
        if (subVerb != "init") {
            Console.Error.WriteLine("Usage: clrkernel-studio git init [--notebooks <dir>]");
            return 2;
        }
        try {
    ...
    internal static string MissingStoreError(JobsOptions options) =>
        options.IsExplicit("store") ? null :
            ...
            serve needs an explicit run-history store.
              ...
              --store postgres     needs --connection-string (or CLRKERNEL_STUDIO_CONNECTION)
              --store sqlserver    needs --connection-string (or CLRKERNEL_STUDIO_CONNECTION)

            Set it with --store, CLRKERNEL_STUDIO_STORE, or "store" in settings.json.
    ...
    private static async Task<int> ServeAsync(ProjectRegistry projects, JobsOptions options) {
        ...
        Console.WriteLine($"clrkernel-studio scheduler — {result.Jobs.Count} job(s) in " +
    ...
    internal static WebApplication BuildApp(
        ...
        settings.Add(new SettingsSection {
            ...
            Description = options.GitEnabled
                ...
                : "Off. Run `clrkernel-studio git init` on the notebooks root to enable test→prod promotion.",
            Fields = {
                new SettingField {
                ...
                new SettingField {
                    ...
                    Value = options.GitAuthorName ?? "clrkernel-studio",
                ...
                new SettingField {
                    ...
                    Value = options.GitAuthorEmail ?? "studio@clrkernel.local",
