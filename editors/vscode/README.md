...
3. Run a cell. The first time, if `ClrKernel` isn't found the extension
   ...
   (`dotnet tool install --global ClrKernel --version 0.12.*`).
   Prefer to do it yourself? Run that command in a terminal ahead of time —
   include the `--version`, because this build talks to **kernel 0.12.x** and
...
- **ClrKernel 0.12.x** — on PATH as a dotnet tool, or configured via settings.
