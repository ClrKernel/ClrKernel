...
public partial class AddIdentities : Migration {
    protected override void Up(MigrationBuilder migrationBuilder) {
        ...
        migrationBuilder.CreateIndex(
            ...
            table: "identities",
            column: "user_id");
        // The id has to be shaped the way EF's sqlite provider writes a Guid —
        // upper-case, dashed — or the row it inserts is one nothing can read back.
        // `hex(randomblob(16))` alone is 32 characters with no dashes.
        //
        // Idempotent through the NOT EXISTS, not through the id: running it twice
        // inserts nothing the second time because the passkey already has an
        // identity, which is also what makes it safe on a database that has been
        // upgraded once already.
        migrationBuilder.Sql(
            "INSERT INTO identities (id, provider, subject, user_id, label, created_at) " +
            "SELECT upper(substr(hex(randomblob(4)),1,8) || '-' || substr(hex(randomblob(2)),1,4) " +
            "|| '-' || substr(hex(randomblob(2)),1,4) || '-' || substr(hex(randomblob(2)),1,4) " +
            "|| '-' || substr(hex(randomblob(6)),1,12)), " +
            "'passkey', c.id, c.user_id, c.name, c.created_at " +
            "FROM credentials c " +
