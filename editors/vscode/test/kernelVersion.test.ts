...
describe('kernelVersionWarning', () => {
    it('tells the user what is installed and how to get back', () => {
        const warning = kernelVersionWarning('newer', '0.14.0.0');
        expect(warning).toContain('0.14.0.0');
    ...
    it('says cells still run, because they do — only connection management breaks', () => {
        expect(kernelVersionWarning('newer', '0.14.0.0')).toContain('still run');
    });

    it('asks for an update when the kernel is behind', () => {
        expect(kernelVersionWarning('older', '0.12.0.0')).toContain('dotnet tool update');
    });

    it('stays quiet when the pair is fine or unknown', () => {
        expect(kernelVersionWarning('ok', '0.13.1.0')).toBeUndefined();
