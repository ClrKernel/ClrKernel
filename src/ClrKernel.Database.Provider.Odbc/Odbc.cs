...
/// <summary>
/// ODBC provider entry point. Returns the same fluent <see cref="DataSource"/> as the
...
public static class Odbc {
    ...
    public static DataSource FromConnectionString(string connectionString, string name = "odbc") {
    ...
    public static DataSource FromDsn(string dsn, string user = null, string secretRef = null, SecretStore secrets = null) {
    ...
    /// <c>Server</c>, <c>DataSource</c>). A user's password resolves from the secret store.
    /// </summary>
    ...
    private static DataSource Build(string name, string connectionString) =>
        new DataSource(name, () => new OdbcConnection(connectionString));
}
