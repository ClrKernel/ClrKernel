...
/// <summary>
/// A reference to a table on a <see cref="Database"/>. Reads as a query source
...
public sealed class DatabaseTable {
    internal DatabaseTable(Database database, string name) {
    ...
    public DatabaseQuery Query() => Database.Query($"select * from {Name}");
    ...
    public long Count() => Database.Scalar<long>($"select count(*) from {Name}");
